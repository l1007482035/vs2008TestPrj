...
int GetCurrentWordVersion()
{
	...
	LPCSTR lpSubKey = "Word.Application\\CurVer";
	if (RegOpenKeyExA(hKey, lpSubKey, 0, KEY_QUERY_VALUE, &hKey) != ERROR_SUCCESS)
		return nVer;
	LPCSTR lpValueName = NULL;
	DWORD dwType = REG_SZ;
	char szVer[MAX_PATH] = "";
	DWORD  cbValue = MAX_PATH;
	if (RegQueryValueExA(hKey, lpValueName, NULL, &dwType, (LPBYTE)szVer, &cbValue) != ERROR_SUCCESS)
		return nVer;
	CStringA strVer = szVer;
