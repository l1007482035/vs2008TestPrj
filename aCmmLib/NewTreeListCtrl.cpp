...
void CNewTreeListCtrl::OnCheckItem(HTREEITEM hItem, int nNewCheck, BOOL bFromChild)
{
	...
	{
		//向上遍历，若还有一个被选中，父节点childcheck，否则uncheck


		SetParentCheck(GetParentItem(hItem), TRUE);
		//向下遍历，所有子节点uncheck
		if (!bFromChild)
	...
	{
		//本节点被选中
		//向上遍历，如果所有兄弟节点都被选中，父节点check，否则childcheck\
...
void CNewTreeListCtrl::SetParentCheck(HTREEITEM hParent, BOOL bFromChild)
{
	if (hParent == NULL)
		return;
	...
	while(hItem != NULL)
	{
		if (GetItemCheck(hItem) == ITEM_UNCHECK)
			bHasUncheck = TRUE;
		else if (GetItemCheck(hItem) == ITEM_CHECK)
			bHasCheck = TRUE;
		else 
	...
	if (bHasUncheck && bHasCheck)
		SetItemCheck(hParent, ITEM_CHILDCHECK, bFromChild);
	else if (bHasCheck)
		SetItemCheck(hParent, ITEM_CHECK, bFromChild);
	else 
		SetItemCheck(hParent, ITEM_UNCHECK, bFromChild);
}
