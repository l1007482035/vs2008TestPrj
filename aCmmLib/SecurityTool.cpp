...
BOOL CSecurityTool::GetCurUserSID(CString& sSID)
{
	...
	CString sAccount;
	BOOL bRet = FALSE;
	sAccount.Format("%s\\%s",szCmpName,sUser);
...
{
	theLog.Write("CSecurityTool::GetProcessUser,1");
	BOOL fResult  = FALSE;
    HANDLE hProc  = NULL;
	HANDLE hToken = NULL;
	...
	__try
	{
        ...
        if (hProc == NULL)
		{
			theLog.Write("!!CSecurityTool::GetProcessUser,2,OpenProcess,fResult=%d",fResult);
			__leave;
		}
        fResult = OpenProcessToken(hProc, TOKEN_QUERY, &hToken);
        if(!fResult)  
		{
			theLog.Write("!!CSecurityTool::GetProcessUser,3,OpenProcessToken,fResult=%d",fResult);
			__leave;
		}
		...
		if (dwNeedLen > 0)
		{
			...
			if (!fResult)
			{
				theLog.Write("!!CSecurityTool::GetProcessUser,4,GetTokenInformation,fResult=%d",fResult);
				__leave;
			}
		}
		else
		{
			theLog.Write("!!CSecurityTool::GetProcessUser,5,GetTokenInformation,fResult=%d",fResult);
			__leave;
		}
		...
		theLog.Write("CSecurityTool::GetProcessUser,6,LookupAccountSid,fResult=%d",fResult);
	}
	__finally
	{
		if (hProc)
...
DWORD CSecurityTool::CreateProcByCurUsr(char * pProc,char * pParam,int nShowType,DWORD dwSessionId/* = 0*/)
{
    DWORD bRet = FALSE;
	HANDLE hSnapshot = NULL;
...
DWORD  CSecurityTool::CreateProcUseProcToken(DWORD dwProcessID,char * pProcName,char * pParam,int nShowType,void* pEn)
{
	...
	__try
	{
		...
		{
			sprintf(strMsg,"ERROR=%d",GetLastError());
		...
		int nLen = strlen(pProcName)  + ( pParam ? strlen(pParam) : 0 ) + 10;
		...
		sprintf(pszPath,"%s %s",pProcName,pParam ? pParam : "");
		...
		CCommonFun::WriteFileLog(" err %d",ierror);
...
BOOL CSecurityTool::CreateProcByWinlogon(char *szProcPathName,char* pParam,DWORD dwSessionID)
{
