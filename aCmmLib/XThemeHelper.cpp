								  ...
								  const RECT *pRect)
{
	...
	if (m_bThemeLibLoaded && hTheme)
	{
		...
		int nLen = MultiByteToWideChar(CP_GB2312, MB_PRECOMPOSED, lpszText, _tcslen(lpszText)+1, NULL, 0);
		nLen += 2;
		WCHAR * pszWide = new WCHAR[nLen];
		if (pszWide)
		{
			MultiByteToWideChar(CP_GB2312, MB_PRECOMPOSED, lpszText, _tcslen(lpszText)+1, pszWide, nLen);
			hr = m_DrawThemeText(hTheme, hdc, iPartId, iStateId,
...
HTHEME CXThemeHelper::OpenThemeData(HWND hWnd, LPCTSTR lpszClassList)
{
	...
	if (m_bThemeLibLoaded)
	{
		...
		int nLen = MultiByteToWideChar(CP_GB2312, MB_PRECOMPOSED, lpszClassList, 
			_tcslen(lpszClassList)+1, NULL, 0);
		nLen += 2;
		WCHAR * pszWide = new WCHAR [nLen];
		if (pszWide)
		{
			MultiByteToWideChar(CP_GB2312, MB_PRECOMPOSED, lpszClassList, 
