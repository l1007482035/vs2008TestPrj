...
{
	theLog.Write("CProcessExp::GetProcessUser，1");
	BOOL bResult = FALSE;
	HANDLE          hToken = NULL;
	PTOKEN_USER     pTokenUser = NULL;
	TCHAR            szName[256];
	...
	do
	{
		...
		{
			theLog.Write("!!CProcessExp::GetProcessUser,2,LookupAccountSid is fail");
		}
	}while(0);
	if (pTokenUser != NULL)
	{
...
BOOL CProcessExp::InjectDll(DWORD dwTargetPID, char* szDllPath)
{
	...
	log.Write("handle = 0x%x", hProcess);
	BOOL bRet = FALSE;
		...
		else
		{
			...
			if (CCommonFun::IsWinVistaOrLater())
			{
				do 
				{
					...
					if( !modNtDll )
					{
						log.Write("failed to get module handle for ntdll.dll, Error=0x%.8x", GetLastError());
						break;
					}
					...
					{
						log.Write("failed to get funtion address from ntdll.dll, Error=0x%.8x", GetLastError());
						break;
					}
					...
					if (hThread == NULL)
					{
						log.Write("NtCreateThreadEx failed, Error=0x%.8x", GetLastError());
						break;
					}
					...
					{
						log.Write("Remote thread returned with status = %d", dwExitCode);
					}
...
BOOL ReadUserEnvirnment(DWORD dwSession, _ENVSTRING_t& stEnvData)
{
	...
	//theLog.Write("Read session:%d, %s", dwSession, szExplorerPath);
	BOOL bRet =  CProcessExp::ReadEnvironmentBlock(szExplorerPath, dwSession, stEnvData);
	//theLog.Write("Read env over");
	return bRet;
}
BOOL	CProcessExp::CreateProcessByCurUser(CString sPath,CString sParam,DWORD dwActiveSessionId)
{
	//theLog.Write("CProcessExp::CreateProcessByCurUser,1,sPath=%s,dwActiveSessionId=%d", sPath, dwActiveSessionId);
	HANDLE hToken = NULL;
	HANDLE hToken2 = NULL;
	...
	//theLog.Write("CProcessExp::CreateProcessByCurUser,2,dwActiveSessionId=%d", dwActiveSessionId);
	try
	{
		...
		{
			theLog.Write("Find Token error");
			goto ERR0;
		}
		else
		{
			//theLog.Write("find token successfully");
			BOOL bRet = DuplicateTokenEx(hToken,0,NULL,SecurityAnonymous,TokenPrimary,&hToken2);//复制一下Token才能创建用户进程成功
			if(bRet)
			{
				//theLog.Write("DuplicateTokenEx success");
			}
			else
			{
				//sprintf(cMsg,);
				theLog.Write("DuplicateTokenEx error:%d",GetLastError());
				goto ERR1;
			}
		}
		...
		if (bDefaultDesktop)
		{
			...
			{
				theLog.Write("default: failed to read user envirnment.");
			}
			else
			{
				pEn = stEnvData.pData;
		...
		//theLog.Write("env size:%d", stEnvData.nSize);

		WCHAR wcFileName[MAX_PATH * 2] = {0};
		int nRet = MultiByteToWideChar(CP_GB2312,0,cFileName,strlen(cFileName)+1,wcFileName,MAX_PATH * 2);
		if(nRet)
		{
			...
			if(!CreateProcessAsUserW(
				...
				&pinfo) )
			{
				theLog.Write("use wide char to create failed  error:%d 文件名：%s",::GetLastError(),cFileName);
			}
			else
			{
				//theLog.Write("use wide char to create  successfully, sessionid: %d", dwActiveSessionId);
				bCreatSuccess = true;
			}
		}
		...
		if(!bCreatSuccess)
		{
			...
			{
				//sprintf(cMsg,);
				theLog.Write("user multi char to create error:%d 文件名：%s",::GetLastError(),cFileName);
			}
			else
			{
				bCreatSuccess = true;
				//theLog.Write("user multi char to create eguard.exe successfully");
	...
	{
		//sprintf(cMsg,);
		theLog.Write("catch2 error:%d",::GetLastError());
		goto ERR2;
	}
