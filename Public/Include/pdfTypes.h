...
struct ZPrintParams
{
    ...
    char cPrinterName[MAX_PATH]; // ansi 编码
    char cDocName[MAX_PATH]; // ansi 编码
    int nBeginPage;
    int nEndPage;
    int nPagesPerPaper;
    int nResolution_x;// in PPI
    int nResolution_y;// in PPI
    WORD wColor;
    WORD wPaperSize;
    WORD wOrientation;
    WORD wCopies;
    ...
    BOOL bPauseJob;
    int* pJobId; ///< 打印机提交的作业id, 输出参数
    BOOL bImagePrint; ///< 是否以光栅图打印, 即先把pdf页面转成图片后再打印, 缺点是较慢
};
